...
// --- Module C Extension: Attendance & Rostering ---

export interface WorkShift {
  id: string;
  name: string; // e.g. "早班 (A)", "晚班 (B)"
  code: string; // "D", "N", "OFF"
...
export interface AttendanceRecord {
  ...
  status: 'NORMAL' | 'LATE' | 'EARLY_LEAVE' | 'ABSENT' | 'OT';
  work_hours: number;
}
...
// --- Module F: Performance Management Center ---
...
export interface KeyResult {
  ...
  target_val: number;
  current_val: number;
  linked_project_id?: string; // Auto-Sync Link
}
...
export interface ReviewSession {
  ...
  type: ReviewType;
  status: 'PENDING' | 'SUBMITTED' | 'SIGNED_OFF';
  score?: number;
...
// --- Module G: Asset Management Center ---

export type AssetStatus = 'IN_STOCK' | 'ASSIGNED' | 'REPAIR' | 'LOST' | 'DISPOSED';
export type AssetType = 'HARDWARE' | 'SOFTWARE' | 'LICENSE';

export interface Asset {
  id: string;
  tag_id: string; // QR Code Content
  name: string;
  type: AssetType;
  status: AssetStatus;
  location_id: string;
...
export interface AssetEvent {
  id: string;
  asset_id: string;
  type: 'AUDIT' | 'REPAIR' | 'LOST' | 'CHECK_IN' | 'CHECK_OUT';
  date: string;
  user_id: string;
  description: string;
}
