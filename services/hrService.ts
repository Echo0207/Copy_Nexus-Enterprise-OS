...
import { User, LeaveBalance, Payslip, LeaveType, LeaveRequest, PayslipItem, CostTransaction, Document, MonthlyCostReport, ProjectMonthlyDetail, ProjectTaskCost, ProjectPersonCost, WorkShift, RosterEntry, AttendanceRecord } from '../types';
...
import { MOCK_SHIFTS, MOCK_ROSTER, MOCK_ATTENDANCE } from './mockHRData';
...
export const hrService = {
  ...
  // --- Attendance Module (Module C Extension) ---
  ...
  getAttendanceLogs: async (userId: string, date: string): Promise<AttendanceRecord | null> => {
    await delay(200);
    return MOCK_ATTENDANCE.find(a => a.user_id === userId && a.date === date) || null;
  },

  punchClock: async (userId: string): Promise<AttendanceRecord> => {
    await delay(500);
    const today = new Date().toISOString().split('T')[0];
    const nowISO = new Date().toISOString();
    
    let record = MOCK_ATTENDANCE.find(a => a.user_id === userId && a.date === today);
    ...
    if (record) {
      ...
      record.work_hours = (end.getTime() - start.getTime()) / (1000 * 60 * 60);
    } else {
      // Check In
      record = {
        id: 'att-' + Date.now(),
        ...
        date: today,
        check_in: nowISO,
        status: 'NORMAL', // Logic to check against Shift start time would go here
        work_hours: 0
      };
  ...
  updateRoster: async (entries: RosterEntry[]): Promise<void> => {
    await delay(600);
    entries.forEach(entry => {
      const idx = MOCK_ROSTER.findIndex(r => r.id === entry.id);
      if (idx !== -1) {
        MOCK_ROSTER[idx] = entry;
      } else {
        MOCK_ROSTER.push(entry);
      }
    });
  }
};
